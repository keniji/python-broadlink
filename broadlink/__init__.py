...
def gendevice(devtype, host, mac):
  devices = {
          ...
          ws2: [0x4F19], # WS2 (Broadlink / Honyar 86 type Wall Plug)
          }
...
class ws2(device):
  ...
  def set_power(self, state):
    """Sets the power state of the wall plug."""
    packet = bytearray(16)
    packet[0x00] = 0x0d
    packet[0x02] = 0xa5
    packet[0x03] = 0xa5
    packet[0x04] = 0x5a
    packet[0x05] = 0x5a
    packet[0x06] = 0xb2 + (0x02 if state else 0x01)
    packet[0x07] = 0xc0
    packet[0x08] = 0x02
    packet[0x0a] = 0x03
    packet[0x0d] = 0x01
    packet[0x0e] = 0x01 if state else 0
  ...
  def check_power(self):
    """Returns the power state of the wall plug."""
    packet = bytearray(16)
    packet[0x00] = 0x0a
    packet[0x02] = 0xa5
    packet[0x03] = 0xa5
